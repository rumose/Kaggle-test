...
%
\title{Sentiment Analysis on Movie Reviews} %总标题
\author{
Yuhui Mou
\\
...
cf={Sentiment Analysis on Movie Reviews}, %每页下面的小标题
}
...
\twotonebox {\rotatebox{90}{sentiments}}{\parbox{.88\textwidth}
{
...
% % \end{itemize}
% }
\emph{Summary}:\textcolor{orange}{Classify the sentiment of sentences from the Rotten Tomatoes dataset} \\
Every years,there are many movies appear on the screen.We can read comments to know a movie is good or not \\
Movie Reviews come from varying people.Some may say the positive reviews,others may say the negative comments
of the movies.We can classify a movie by it's comments.But the reviews is a large dataset,people can't read
every comments.\\
Here it is,we can use computer to classify the dataset.\\
\textcolor{red}{According to the reviews to distinguish sentiments.}
}}
...
\begin{slide}[toc=,bm=]{Data}
  ...
  The dataset is comprised of tab-separated files with phrases from the Rotten Tomatoes dataset.
   ...
   The date is stored in tsv format.
  \begin{itemize}
      \item  train.tsv - contains the phrases and their associated sentiment labels. We have 
      additionally provided a SentenceId so that you can track which phrases belong to a single sentence.
      \item  test.tsv - contains just phrases. You must assign a sentiment label to each phrase.
  \end{itemize}
  In the test file test.tsv, the format of a Sentence is the same as train.tsv, only the sentiment type is removed, 
  because it is the target variable you are going to predict.
  }}
  \end{slide}
%%==========================================================================================
%%
...
%   \centering
%   \selectcolormodel{rgb}
%   \includegraphics[width=0.35\textwidth]{D://Code//Kaggle-test//Data//train.eps}\\
%   \caption{Train.json}
% \end{figure}
\begin{figure}[htbp]
  \centering
  ...
  \centering
  \includegraphics[width=7cm]{D://Kaggle-test//Data////train.eps} 
  \caption{train-data}
  \end{minipage}
  \begin{minipage}[t]{0.48\textwidth}
  \centering
  \includegraphics[width=7cm]{D://Kaggle-test//Data////test.eps}
  \caption{test-data}
  \end{minipage}
  \end{figure}
Each training data has four parts:PhraseId	SentenceId	Phrase	Sentiment.
The features of the reviews come form phrase,
and the training goal is Sentiment.
}
% Outlying Aspects Mining
% \begin{itemize}
% \item
...
\item 
Build a corpus. \\
The text contents of the training set and test set are merged together 
using the contcat function in Pandas
\item 
Import the stop word library\\
We use words like a, an, and, or, of, at, the and so on. 
The information about these words is extremely limited.Therefore, what we need to do is to remove the pause words in the text in the NLP analysis process. The advantage 
of doing this is that we reduce the vocabulary and then reduce the dimension of our feature vector
\end{itemize}
%  \begin{itemize}
%   \item
...
%%==========================================================================================
%%
\begin{slide}{Step Two}
Characteristics of the engineering\\
% \centering\color{red}{\emph{{RandomForestClassifier}}}
\begin{itemize}
\item
Bag-Of-Words model\\
BoW early in Natural Language Processing and Information Retrieval This
model ignored the grammar and word order elements such as text, just as it is a
collection of several words, the emergence of each word in the document are
independent of each otherBoW to use an unordered list of words to express a
text or a document.
\item
TF-IDF model\\
...
%   \selectcolormodel{rgb}
%   \includegraphics[width=0.4\textwidth]{D://Code//Kaggle-test//Data//cidai.eps}\\
%   \caption{Train.json}
% \end{figure}
% \begin{figure}
%   \selectcolormodel{rgb}
%   \includegraphics[width=0.5\textwidth]{D://Code//Kaggle-test//Data//file.eps}\\
%   \caption{out-file}
% \end{figure}
% \begin{figure}[htbp]
%   \centering
%   \begin{minipage}[t]{0.48\textwidth}
%   \centering
%   \includegraphics[width=6cm]{D://Kaggle-test//Data//cidai.eps}
%   \caption{Construct word bag}
%   \end{minipage}
%   \begin{minipage}[t]{0.48\textwidth}
%   \centering
%   \includegraphics[width=6cm]{D://Kaggle-test//Data//file.eps}
%   \caption{Output}
%   \end{minipage}
%   \end{figure}
%%==========================================================================================
...
\begin{slide}{Step Three}
  ...
  \begin{itemize}
    \item Feature engineering the text in the test set.
    \item Logical regression classifier is used to predict the text in the test set.
    \item Format it and save it as a.csv file
  \end{itemize}
...
\begin{slide}[toc=,bm=]{}
  ...
  \begin{figure}[htbp]
    ...
    \begin{minipage}[t]{0.48\textwidth}
    \centering
    \includegraphics[width=4cm]{D://Kaggle-test//Data//output.eps} 
    \caption{outp-data}
    \end{minipage}
  
    \end{figure}
    This is a screenshot of the output file with the 
    sentence ID in the first column and the emotion ID in the second column.\\
    0 - negative;
    1 - somewhat negative;
    2 - neutral;
    3 - somewhat positive;
...
\begin{slide}[toc=,bm=]{}
  ...
  \begin{figure}[htbp]
    ...
    \begin{minipage}[t]{0.48\textwidth}
    \centering
    \includegraphics[width=10cm]{D://Kaggle-test//Data//cnn.eps}
    \caption{CNN}
...
\item
\smallskip
I have knew how to deal with large text class data set----\textcolor{orange}{BoW model}.
\item
\smallskip
I have learned a classifier-\textcolor{orange}{LR}:can predict the test data and classify test data.
\item
\smallskip
The \textcolor{orange}{CNN} network model can also be used to deal with NLP problems.
...
% \item
% \smallskip
% I can learn about the\textcolor{orange}{eating habits}
...
% \item 
% \smallskip
% Know how to deal with large text class data set----\textcolor{orange}{BoW model}.
% \item
% \smallskip
% Learn a classifier-RandomForestClassifier:can predict the test data and classify test data.
% \item
% \smallskip
% But this method still has some drawbacks,such as test data just have txt,it's a single data.And,I just used
% one method to solve this problem.When I extracted the feature of the data and classified the train date,I
% just used CountVectorizer and RandomForestClassifier.This is the disadvantages of my project.
% \end{itemize}
